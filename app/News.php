...
class News extends Model
{
    ...
    public static function getTrending($take = 4)
    {
        ...
        $model = Cache::remember('getTrending4', $expiresAt, function () use ($take, $from, $to) {
            return self::hydrate(DB::select('select t1.* FROM news t1 join (select category_id, MAX(published_at) published_at FROM news GROUP BY category_id) t2 ON t1.category_id = t2.category_id AND t1.published_at = t2.published_at left join popularity_stats ps on ps.trackable_id = t1.id WHERE t1.published_at between ? and ? and ps.all_time_stats != 0 and t1.publish = 1 and t1.deleted_at is null order by ps.all_time_stats DESC LIMIT 4', [$from, $to]));
        });
